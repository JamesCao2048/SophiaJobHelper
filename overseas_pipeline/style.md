...
>
> **关于模板内容**：`templates/` 下各文件已填入 Sophia 的**真实申请材料**作为内容参考（Research Statement 含完整图片、Cover Letter 使用 King's College London 版本）。这些内容本身是正确的——调整样式（字号、字体、边距）后，以此内容为基础生成新材料即可。

### 当前实测页数（含真实内容 + 完整图片）

| 文档 | 原始页数（原样式） | 当前模板页数（新样式） | 目标页数 | 状态 |
|------|-----------------|-------------------|---------|------|
| **Cover Letter** | — | **2 页** | 2 页 | ✅ |
| **Research Statement** | 5 页（含 4 张图） | **8 页** | 4 页 | ⚠ +3 |
| **Teaching Statement** | — | **3 页** | 2 页 | ⚠ +1 |
| **Diversity Statement** | — | **1 页** | 1–2 页 | ✅ |

> **请打开 `templates/` 下对应的 PDF 查看实际效果，确认样式后再决定是否需要调整字号/字体。**

### 原因

新样式使用 **lmodern（Computer Modern）** 字体以与 CV 风格统一，但该字体比原始模板所用的 **mathptmx（Times）** 更宽，导致相同内容需要更多页面。Research Statement 额外受 4 张插图影响，膨胀更明显（原始 5 页 → 新样式 8 页）。
