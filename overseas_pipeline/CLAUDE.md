...
**❌ 禁止** 直接使用 `WebFetch` 工具或手动 `curl` 命令
**✅ 统一调用 `web_fetch_utils.py` CLI**（工作目录为 `overseas_pipeline/`）：

```bash
# 基本用法
python src/web_fetch_utils.py "https://example.com/page"

# 带 Layer 3 搜索提示（Tavily Search fallback 使用）
python src/web_fetch_utils.py "https://example.com/page" --search-hint "University CS faculty list"
```

**输出格式**：
- 第一行：`<!-- fetched via: {layer} -->`（layer1_curl / layer1.5_jina / layer2_tavily_extract / layer2.5_wayback / layer3_tavily_search）
- 后续行：页面内容

脚本自动依次尝试五层 fallback，全部失败才报错。`page_scraper.py`、`faculty_scraper.py`、`course_catalog_scraper.py` 内部已调用同一 `fetch_with_fallback()` 函数；**脚本报告 `⚠ failed URLs` 后**，Claude Code 对这些 URL 用上述 CLI 命令补救。

## 资源引用关系

```
overseas_pipeline/           ← 本模块（所有资源自包含）
├── src/page_scraper.py      ← Step 1 爬取院系页面 & JD
├── src/faculty_scraper.py   ← Step 1 教授个人信息抓取 & 论文下载
...
│   │   └── step1d_regional_signals.md ← Phase D: 地区信号检测（条件触发）
│   ├── step2_analysis.md
│   ├── step3_materials.md
...
│       ├── humanizer_checklist.md     ← Humanizer 检查清单
...
**触发词**：清空、重置、删除数据、重新来过、clear + 学校名（+ 可选院系名）

**执行流程：**

1. 推断 `school_id`（+ 可选 `dept_id`）
2. 列出将被删除的目录/文件，**展示给用户确认**：
   - 单院系模式（school + dept 均指定）：
     - `output/{school_id}/{dept_id}/`
     - `../region_knowledge/schools/{school_id}/{dept_id}.md`（如存在）
   - 全校模式（仅指定 school）：
     - `output/{school_id}/`（整个学校目录）
     - `../region_knowledge/schools/{school_id}/`（整个学校目录，如存在）
3. **必须等用户回复"确认"/"Y"/"yes" 后才执行删除**，不可静默删除
4. 执行删除：
   ```bash
   # 单院系
   rm -rf output/{school_id}/{dept_id}
   rm -f ../region_knowledge/schools/{school_id}/{dept_id}.md

   # 全校
   rm -rf output/{school_id}
   rm -rf ../region_knowledge/schools/{school_id}
   ```
5. 删除后报告：哪些目录/文件已清空，哪些原本不存在（跳过）

**注意：**
- **不删除** `../region_knowledge/regions/{region}.md`（地区规则卡是共享知识，不属于单校数据）
- 同时更新 tracking 数据库状态（如该学校有 app 记录，重置为 `discovered` 状态）：
  ```python
  tracker.reset_to_discovered(app_id=app_id)  # 若 tracker 支持此操作
  ```
